jQuery(document).ready(function($){
  ...
  if(getMapdata!=undefined){
		...
		if(mapdataObj.covid_us_timeline!=undefined){
			covid_us_timeline.push(mapdataObj.covid_us_timeline);
		}
		if(mapdataObj.covid_us_total_timeline!=undefined){
      //console.log(mapdataObj.covid_total_timeline);
			covid_us_total_timeline.push(mapdataObj.covid_us_total_timeline);
    }
    //console.log(covid_us_timeline);
		generateChart(covid_us_total_timeline,covid_us_timeline);
  }
  function generateChart(covid_us_total_timeline,covid_us_timeline){
...
var numberFormatter = new am4core.NumberFormatter();
    ...
    var list = covid_us_timeline[0].list;
    //  console.log(list);
    ...
    chartAndSliderContainer.disabled = true;
    ...
    function updateMapData(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (image) {
          // console.log(image.dataItem.dataContext.id);
          var population = image.dataItem.dataContext.id;
  ...
  function handleHover(event){
    ...
    function updateCountryTooltip() {
      ...
      polygonSeries.mapPolygons.template.tooltipText = "[bold]{name}: {testsPerOneMillion.formatNumber('#.')}[/]\n[font-size:10px] Test per million"
  ...
  function populateCountries(list) {
    var table = $(".cvct-basic-map-table tbody");
        ...
        var area = list[i];
          ...
          $(".cvct-basic-map-table").DataTable({
              "paging": false,
              "select": true
          }).column("1")
              .order("desc")

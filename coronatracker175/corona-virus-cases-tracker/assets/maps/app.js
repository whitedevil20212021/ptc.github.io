...
jQuery(document).ready(function($){
	...
	var getMapdata=$("#cvct-map-data").html();
	var covid_total_timeline = [];
	
	if(getMapdata!=undefined){
		var mapdataObj=JSON.parse(getMapdata);
		if(mapdataObj.covid_world_timeline!=undefined){
			covid_world_timeline.push(mapdataObj.covid_world_timeline);
		}
		if(mapdataObj.covid_total_timeline!=undefined){
			covid_total_timeline.push(mapdataObj.covid_total_timeline);
		}
		generateChart(covid_total_timeline,covid_world_timeline);
	}
	...
	var request = $.ajax({
	  ...
	  request.done(function (response, textStatus, jqXHR ){
			if(response!=undefined){
				 covid_world_timeline.push(JSON.parse(response));
...
function generateChart(covid_total_timeline,covid_world_timeline){
	var cvctMaptitle=$("#cvct_wrapper").attr("data-title");
// Themes begin
...
am4core.ready(function() {
	...
	var container = am4core.create("cvct_map", am4core.Container);
	...
	buttonsContainer.layout = "vertical";
	...
	chartAndSliderContainer.height = am4core.percent(0);
	chartAndSliderContainer.width = am4core.percent(0);
...
*/
	...
	lineChart.fontSize = "0";
	lineChart.paddingRight = 0;
	lineChart.paddingLeft =0;
	
	lineChart.maskBullets = false;
	lineChart.zoomOutButton.disabled = true;
	lineChart.paddingBottom = 0;
...
/*	lineChart.cursor = new am4charts.XYCursor();
	...
	*/
	...
	var buttons = {  confirmed: confirmedButton, recovered: recoveredButton, deaths: deathsButton,active: activeButton };
	...
	function setCountryData(countryIndex) {
		...
		for (var i = 0; i < lineChart.data.length; i++) {
			...
			if (countryData) {
				dataContext.recovered = (countryData.recovered) ? countryData.recovered:"N/A";
				dataContext.confirmed = (countryData.confirmed) ? countryData.confirmed:"N/A";
				dataContext.deaths = (countryData.deaths) ? countryData.deaths:"N/A";
				dataContext.active = (countryData.confirmed - countryData.recovered - countryData.deaths) ? countryData.confirmed - countryData.recovered - countryData.deaths:"N/A";
	...
	function populateCountries(list) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			$("<td>").appendTo(tr).data("areaid", area.id).html((area.name) ? area.name:'N/A');
			$("<td>").addClass("value").appendTo(tr).html((area.confirmed) ? area.confirmed:'N/A');
			$("<td>").addClass("value").appendTo(tr).html((area.deaths) ? area.deaths:'N/A');
			$("<td>").addClass("value").appendTo(tr).html((area.recovered) ? area.recovered:'N/A');
	...
	setTimeout(function(){
		$( "#cvct_wrapper table tbody tr:first-child").trigger( "click" );
	},2000);

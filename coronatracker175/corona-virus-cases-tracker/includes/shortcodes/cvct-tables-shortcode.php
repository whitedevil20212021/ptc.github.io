...
$cun_today_death = [];
	...
	if(is_array($stats_data) && count($stats_data)>0) {	
		$global_confirmed_sum = !empty($stats_data['total_cases'])?$stats_data['total_cases']:'N/A';
		$global_recovered_sum = !empty($stats_data['total_recovered'])?$stats_data['total_recovered']:'NA/';
		$global_death_sum = !empty($stats_data['total_deaths'])?$stats_data['total_deaths']:'N/A';
		$global_today_death = !empty($stats_data['today_deaths']) ? $stats_data['today_deaths'] : 'N/A';
		$global_today_cases = !empty($stats_data['today_cases']) ? $stats_data['today_cases'] : 'N/A';
		$global_today_recover = !empty($stats_data['todayRecovered']) ? $stats_data['todayRecovered'] : 'N/A';
	...
	if($cvct_get_data==false){
		$cvct_get_data = cvct_get_all_country_alternative();
	}
	...
	if(is_array($cvct_get_data)&& count($cvct_get_data)>0){
		...
		foreach($cvct_get_data as $cvct_stats_data){
			...
			$cun_confirmed = $cvct_stats_data["all_data"]->cases;
			$cun_recovered = $cvct_stats_data["all_data"]->recovered;
			$cun_death = $cvct_stats_data["all_data"]->deaths;
			$cun_active = $cvct_stats_data["all_data"]->active;
			...
			$cun_today_cases[] = !empty($cvct_stats_data["all_data"]->todayCases)?$cvct_stats_data["all_data"]->todayCases:'N/A';
			$cun_today_recovere[] = !empty($cvct_stats_data["all_data"]->todayRecovered)?$cvct_stats_data["all_data"]->todayRecovered:'N/A';

			$cun_today_death []= !empty($cvct_stats_data["all_data"]->todayDeaths)?$cvct_stats_data["all_data"]->todayDeaths:'N/A';
			
			$country_name[] = isset($cvct_stats_data['country'])?$cvct_stats_data['country']:'N/A';
			$country_confirmed[] = !empty($cun_confirmed)?number_format($cun_confirmed):'N/A';
			$country_recovered[] = !empty($cun_recovered)?number_format($cun_recovered):'N/A';
			$country_death[] = !empty($cun_death)?number_format($cun_death):'N/A';
			$country_active[] = !empty($cun_active)?number_format($cun_active):'N/A';
			$country_recovered_p[] = !empty($cun_recovered_p)?number_format($cun_recovered_p,1):'N/A';
			$country_death_p[] = !empty($cun_death_p)?number_format($cun_death_p,1):'N/A';
			$country_active_p[] = !empty($cun_active_p)?number_format($cun_active_p,1):'N/A';
	...
	default:
		...
		for($table_rows = 0; $table_rows < $show_entry; $table_rows++) {
			...
			$title=cvct_get_country_info($c_name);
			$arrow_cases = ($cun_today_cases[$table_rows] == 'N/A') ? 'none' : "";
			$arrow_death = ($cun_today_death[$table_rows] == 'N/A') ? 'none ' : "";
			$arrow_recover = ($cun_today_recovere[$table_rows] == 'N/A') ? 'none' : "";
			if($title == '') { $title = $c_name; }
			$cvct_html.= '
			<tr>	
			<td>'.$title.'</td>
			<td>'.$country_confirmed[$table_rows].'<br><span class="dashicons dashicons-arrow-up-alt" style="font-size:10px;display:'.$arrow_cases.'">'.$cun_today_cases[$table_rows] .'</span></td>
			<td class="table-recovered">'.$country_recovered[$table_rows].'<br><span class="dashicons dashicons-arrow-up-alt"style="font-size:10px;display:'.$arrow_recover.'">'.$cun_today_recovere[$table_rows] .'</span></td>
			<td class="table-death">'.$country_death[$table_rows].'<br><span class="dashicons dashicons-arrow-up-alt" style="font-size:10px;display:'.$arrow_death.'">'.$cun_today_death[$table_rows] .'</span></td>
			</tr>
			';
		}
		$cvct_html.= '
		...
		<th>'.number_format($global_confirmed_sum).'<br><span class="cvct_today">'.$global_today_cases.'</span></th>
		<th>'.number_format($global_recovered_sum).'<br><span class="cvct_today">'.$global_today_recover.'</span></th>
		<th>'.number_format($global_death_sum).'<br><span class="cvct_today">'.$global_today_death.'</span></th>
		</tr>
		</tfoot>
		</table>

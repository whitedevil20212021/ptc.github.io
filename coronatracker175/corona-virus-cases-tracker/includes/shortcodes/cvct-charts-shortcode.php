...
class CVCT_Charts_Shortcode
{
    ...
    public function __construct()
    {
        ...
        add_shortcode('cvct-rate-distribution-charts', array($this, 'cvct_rate_distribution_charts_shortcode'));
    }
 ...
 */
    public function cvct_charts_shortcode($atts, $content = null)
    {
      ...
      margin: 35px auto;';
        $jsonData = '';
        if ($country == "US") {
            $jsonData = $this->cvct_get_us_state_data($type, $show_data);
        } else {
            $jsonData = $this->cvct_get_all_countries_data($type, $show_data);
        }
...
|--------------------------------------------------------------------------
 */
    public function cvct_get_us_state_data($charttype, $show_data)
    {
        $data_arr = [];
        $restData = 0;
        $restData_death = 0;
        ...
        if ($usStates == false) {
            $usStates = cvct_get_us_states_data('USA');
        }
...
|--------------------------------------------------------------------------
 */
    public function cvct_get_all_countries_data($charttype, $show_data)
    {
        $data_arr = [];
        $restData = 0;
        $restData_death = 0;
        ...
        if ($allData == false) {
            $allData = cvct_get_all_country_data_alternative();
        ...
        if (is_array($allData) && count($allData) > 0) {
            ...
            foreach ($allData as $countrydata) {
                ...
                if ($i > 10) {
                    ...
                    $reststckdata += is_numeric($countrydata['cases']) ? $countrydata['cases'] : 0;
                    ...
                    $data_arr[] = array("x" => $countrydata["country"], "y" => ($countrydata[$show_data]) ? $countrydata[$show_data] : 'N/A');
...
//country based piechart shortcode
    ...
    public function cvct_rate_distribution_charts_shortcode($atts, $content = null)
    {
        $atts = shortcode_atts(array(
        ...
        $country_name = '';
        if ($allData == false) {
            $allData = ($country_code == "all") ? cvct_get_global_data_alternative() : cvct_country_stats_data_alternate($country_code);
        }
 ...
 */
    public function cvct_get_us_state_code($name)
    {

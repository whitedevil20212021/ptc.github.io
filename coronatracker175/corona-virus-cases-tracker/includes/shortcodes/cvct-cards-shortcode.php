...
public function  cvct_shortcode( $atts, $content = null ) {
    ...
    $title=$atts['title'];
    if($style=="style-4"){
        $title =cvct_get_country_info($country) ." ".esc_html($title);
    }
    ...
    if( $country=="all"){
        ...
        $c_name=cvct_get_country_name($country);
   ...
   $last_updated=date("d M Y, G:i A",strtotime(get_option('cvct_cs_updated'))).' (GMT)';
    ...
    if(is_array($stats_data) && count($stats_data)>0){
        ...
        $today_cases=!empty($stats_data["allData"]->todayCases) ?$stats_data["allData"]->todayCases:(($country=="all")?$stats_data['today_cases']:"N/A");
        $today_death = !empty($stats_data["allData"]->todayDeaths) ? $stats_data["allData"]->todayDeaths :( ($country=="all")?$stats_data['today_deaths']:"N/A");
        $today_recover = !empty($stats_data["allData"]->todayRecovered) ?$stats_data["allData"]->todayRecovered : ( ($country=="all")?$stats_data['todayRecovered']:"N/A");
       $show_today_cases=($today_cases=='N/A')?"none":"";
       $show_today_deaths=($today_death=='N/A')?"none":"";
       $show_today_recovered = ($today_recover == 'N/A') ? "none" : "";
        
        $arrow_cases=($today_cases!=='N/A')?'class="dashicons dashicons-arrow-up-alt" ':"";
        $arrow_death = ($today_death !== 'N/A') ? 'class="dashicons dashicons-arrow-up-alt" ' : "";
        $arrow_recover = ($today_recover !== 'N/A') ? 'class="dashicons dashicons-arrow-up-alt" ' : "";
        if($total>=1){
        ...
        case "style-6":
            ...
            <div class="coronatracker-card cvct-card-6" style="'.esc_attr($custom_style).'"><div class="cvct-card-6-bg"></div>
                ...
                --><div class="cvct-number total_case">
                    <span id="cvct_today_case" '.$arrow_cases.'>'.esc_html($today_cases).'</span> 
                    <span class="large-num">'.esc_html($total_cases).'</span> 
                    <span>'.esc_html($label_total).'</span>
                    </div><!-- 
                --><div class="cvct-number death_case">
                    <span '.$arrow_death.' id="today_death">'.esc_html($today_death).'</span>
                    <span class="large-num">'.esc_html($total_deaths).' <small>('.esc_html($deathPerstange).')</small></span> 
                    <span>'.esc_html($label_deaths).'</span>
                    </div><!-- 
                --><div class="cvct-number recovered_case">
                    <span '.$arrow_recover.' id="today_recover">'.esc_html($today_recover).'</span>
    ...
    $css='<style data-s="'.esc_attr($source).'">'. esc_html($this->cvct_load_styles($style,$show_today_cases,$show_today_deaths,$show_today_recovered)).'</style>';
...
|--------------------------------------------------------------------------
*/  
function cvct_load_styles($style,$show_today_cases,$show_today_deaths,$show_today_recovered){
    $css = "";
    $css="
    #cvct-card-wrapper {
        width: 100%;
    ...
    elseif($style=="style-6"){
        $css.= "
         #cvct-card-wrapper .cvct-card-6 .cvct-number.death_case span:nth-child(1){
           
            display:$show_today_deaths;

              
        }
          #cvct-card-wrapper .cvct-card-6 .cvct-number.recovered_case span:nth-child(1) {
           
             display:$show_today_recovered;

        }
        #cvct-card-wrapper .cvct-card-6 .cvct-number.total_case span:nth-child(1) {
           display:$show_today_cases;
        ...
        #cvct-card-wrapper .cvct-card-6 .cvct-number span:nth-child(3) {
        ...
        #cvct-card-wrapper .cvct-card-6 .cvct-number.death_case span:nth-child(3) {
            background: #c50909;
        }
        
        #cvct-card-wrapper .cvct-card-6 .cvct-number.recovered_case span:nth-child(3) {
            background: #0c9c0c;
        }
          #cvct-card-wrapper .cvct-card-6 .cvct-number span:nth-child(1) {           
            color: #fff;          
        }
         #cvct-card-wrapper .cvct-card-6 .cvct-number.death_case span:nth-child(1) {
            color: #c50909;

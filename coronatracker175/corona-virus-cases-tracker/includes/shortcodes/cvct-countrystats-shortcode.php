...
public function  cvct_country_shortcode( $atts, $content = null ) {
    ...
    if($country_code=="IN"){
      $stats_data= cvct_country_stats_data($country_code);
    }else{
      ...
      switch($layout){
          case 'style-2':
            ...
            if($countrydata["state"]){
              $state_code=$this->cvct_get_us_state_code($countrydata["state"]);
            ...
            unset( $states_data[0]);
             ...
             if($countrydata["state"]){
               $state_code=$this->cvct_get_in_state_code($countrydata["state"]);
               $countrydata['recovered']= $recovered;
               $states_data[$state_code]['all']= $countrydata;
               $states_data[$state_code]['fillKey']=$condition;
             }
 ...
 */
public function cvct_get_in_state_code($name){
...
public function cvct_customCSS(){
    $css='<style>

...
function __construct() {
    ...
    add_action('wp_ajax_basic_map_data', array($this, 'cvct_get_basic_map_data'));
    add_action('wp_ajax_nopriv_basic_map_data', array($this, 'cvct_get_basic_map_data'));
}
      ...
      if(get_option('delete_contries_data_cache')==false){
        ...
        #cvct_basic_map_wrp svg {left:0;}
          ...
          #cvct_basic_map_wrp .dataTables_paginate .paginate_button:hover {
            ...
            $map_json_data= $this->cvct_generate_map_data();
 ...
 */
public function cvct_get_basic_map_data(){
  $jsonData='';
 $data_arr=[];
         ...
         if(is_array($allData)&& count($allData)>0){
             foreach($allData as $countrydata){
               ...
               $countryInfo= $countrydata['all_data']->countryInfo;
                ...
                $deaths= !empty($countrydata["deaths"])?$countrydata["deaths"]:"N/A";
               $confirmed=!empty($countrydata["cases"])?$countrydata["cases"]:"N/A";
              $recovered =!empty($countrydata["recoverd"])?$countrydata["recoverd"]:"N/A";
 ...
 */
 public function cvct_generate_map_data(){
     $jsonData='';
    $data_arr=[];
    $restData='';
    $globalCount['deaths']=0;
      ...
      $allData=cvct_get_all_country_data();
        if($allData==false){
          $allData=cvct_get_all_country_alternative();
            ...
            if(is_array($allData)&& count($allData)>0){
                foreach($allData as $countrydata){
                  $deaths= !empty($countrydata["deaths"])?$countrydata["deaths"]:0;
                  $confirmed=!empty($countrydata["cases"])?$countrydata["cases"]:0;
                 $recovered =!empty($countrydata["recoverd"])?$countrydata["recoverd"]:0;
                  $globalCount['deaths']+=$deaths;
                  $globalCount['confirmed']+=$confirmed;
                  $globalCount['recovered']+=$recovered;
                  $countries[]=$countrydata["country"];
...
function cvct_map_load_assets($layout){
    ...
    if($layout=="style-1"){
          ...
          wp_enqueue_script("cvct_maps",CVCT_URL.'assets/maps/maps.js',null,null,true);
          
          wp_enqueue_script("cvct_animated_js");
          wp_enqueue_script("cvct_dark_theme",CVCT_URL.'assets/maps/dark.js',null,null,true);

          wp_enqueue_script("cvct_worldLow",CVCT_URL.'assets/maps/worldLow.js',null,null,true);
          ...
          wp_enqueue_script("cvct_app",CVCT_URL.'assets/maps/app.js',array('jquery'),null,true);
